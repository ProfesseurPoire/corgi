...
inline float elastic_easing_in_out(float t, float b, float c, float d)
{
    ...
    float postFix = a * corgi::math::pow(2, -10 * (t -= 1));    // postIncrement is evil
...
public:
    ...
    float value(int time)
    {
        ...
        const auto duration = static_cast<float>(segment.time - segment.startTime - 1);
        const auto actualTime = static_cast<float>(time - segment.startTime);
        ...
        switch(segment.easing)
        {
            ...
            case EasingEquation::QuadraticEasingInOut:
                return 0.0f;
