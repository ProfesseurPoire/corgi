...
namespace corgi
{
    ...
    public:
		 ...
		 */
        [[nodiscard]] const Vector<RefEntity>& children() const noexcept;

        /*!
		 * @brief	Returns the entity's children list as a reference
		 */
        [[nodiscard]] Vector<RefEntity>& children() noexcept;
        ...
        Ref<T> add_component(Args&&... args)
        {
            ...
            return scene_->component_maps().get<T>()->add_param(
                _id, std::forward<Args>(args)...);
        }
    ...
    private:
        ...
        Scene* scene_ =
            nullptr;    // We need the reference to the scene to actually delete the entity and its attached components
        ...
        Vector<corgi::RefEntity> children_;
        std::string              name_;
