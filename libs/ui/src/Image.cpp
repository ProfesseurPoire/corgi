...
namespace corgi::ui
{
    ...
    {
        texture_  = image;
        mMaterial = *ResourcesCache::get<Material>("corgi/materials/ui/unlit_ui_sprite.mat");
        if(!texture_)
            return;
    ...
    const Texture* Image::texture() const noexcept { return texture_; }
    ...
    void Image::paint(Renderer& renderer)
    {
        ...
        mMaterial.set_uniform(flip_x_str, 0);
        ...
        renderer.window_draw_list().add_mesh(
            ...
            Matrix::translation(real_x() + (width() - newWidth) / 2.0f, real_y() + (height() - newHeight) / 2.0f, 0.0f),
            mMaterial);
    }
}    // namespace corgi::ui