...
void ui::Window::update_rectangles()
{
    ...
    left_border_ =
        Rect(x_, y_ + offset, offset - 1.0f, height_ - offset * 2.0f - 1.0f);
    right_border_ = Rect(x_ + width_ - offset, y_ + offset, offset - 1.0f,
                         height_ - offset * 2.0f - 1.0f);

    top_border_    = Rect(x_ + offset, y_, width_ - offset * 2.0f - 1.0f, offset - 1.0f);
    bottom_border_ = Rect(x_ + offset, y_ + height_ - offset,
                          width_ - offset * 2 - 1.0f, offset - 1.0f);

    top_left_corner_  = Rect(x_, y_, offset - 1.0f, offset - 1.0f);
    top_right_corner_ = Rect(x_ + width_ - offset, y_, offset - 1.0f, offset - 1.0f);

    bottom_left_corner_ =
        Rect(x_, y_ + height_ - offset, offset - 1.0f, offset - 1.0f);
    bottom_right_corner_ = Rect(x_ + width_ - offset, y_ + height_ - offset,
...
ui::Window::Window()
{
    on_mouse_over_ += [&](const Mouse& mouse)
    {
        const auto fx = static_cast<float>(mouse.x());
        const auto fy = static_cast<float>(mouse.y());
    ...
    mouse_drag_start_event_ += [&](const Mouse& mouse)
    {
        update_rectangles();
    ...
    mouse_drag_end_event_ += [&](const Mouse& mouse)
        { };

    try
    {
        ns.material = *ResourcesCache::get<Material>(
            "corgi/materials/ui/nine_slice.mat");
        auto* t =
            ResourcesCache::get<Texture>("corgi/textures/mpb.tex");
    ...
    try
    {
        _text_title.material = *ResourcesCache::get<Material>(
            "corgi/materials/unlit/unlit_texture.mat");
        font = ResourcesCache::get<Font>(
            "corgi/fonts/comic_sans.fnt");
...
void ui::Window::paint(Renderer& renderer)
{
    ...
    renderer.window_draw_list().add_mesh(UiUtils::nine_slice_quad(), Matrix(),
                                         ns.material, *window_);
    ...
    _text_title.set_text(title_.c_str());
    renderer.window_draw_list().add_mesh(*_text_title.mesh,
                                         Matrix::translation(x_ + 3, y_, 0.0f),
                                         _text_title.material, *window_);
}
}    // namespace corgi::ui
