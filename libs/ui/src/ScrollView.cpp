...
namespace corgi::ui
{
    ScrollView::ScrollView() { isViewport = true; }

    Widget* ScrollView::content() { return content_; }

    void ScrollView::init()
    {
        mMaterial =
            *ResourcesCache::get<corgi::Material>("corgi/Materials/ui/uiRectangle.mat");
        ...
        mVerticalScrollbarHandle->on_mouse_drag_begin() += [&](int x, int y)
        {
            startDragY  = y;
        ...
        mVerticalScrollbarHandle->on_mouse_drag_end() += [&](int x, int y)
        {
            mIsDragging = false;
        };
        ...
        mVerticalScrollbarHandle->on_mouse_drag() += [&](int x, int y)
        {
            mVerticalScrollbarHandle->setColor(selectedHandleColor);
            if(!getChildren().empty())
            {
                ...
                if(ch > h)
                {
                    ...
                    setScrollbarOffset(startOffset + y - startDragY);
                }
            }
        };
    }
    ...
    void ScrollView::paint(corgi::Renderer& renderer)
    {
        ...
        renderer.window_draw_list().add_mesh(
            SpriteRendererSystem::getUiMesh(),
