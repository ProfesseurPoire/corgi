...
namespace corgi::ui
{
    ...
    Event<>& Widget::onEnable() { return onEnable_; }
    Event<>& Widget::onDisable() { return onDisable_; }

    void Widget::destroy() { mIsDestroyed = true; }
    ...
    void Widget::setToolTip(const SimpleString& tooltip)
    {
        if(tooltip == "")
        {
            hasTooltip_ = false;
    ...
    bool Widget::getResizeTop() const noexcept { return resize_.top; }

    bool Widget::getResizeLeft() const noexcept { return resize_.left; }

    bool Widget::getResizeBottom() const noexcept { return resize_.bottom; }
    ...
    const SimpleString& Widget::name() const noexcept { return name_; }

    void Widget::setName(const SimpleString& name) { name_ = name; }

    Widget* Widget::find(const SimpleString& name) const noexcept
    {
    ...
    bool Widget::getResizeRight() const noexcept { return resize_.right; }
    ...
    float Widget::getTop() const noexcept { return y_; }
    ...
    float Widget::getLeft() const noexcept { return x_; }
    ...
    void Widget::setFocusIn() { mManualFocusIn = true; }

    void Widget::setFocusOut() { mManualFocusOut = true; }
    ...
    bool Widget::isActive() const noexcept { return activity_; }
    ...
    bool Widget::isDestroyed() const noexcept { return mIsDestroyed; }
    ...
    float Widget::real_x() const { return parentX_ + x(); }

    float Widget::real_y() const { return parentY_ + y(); }
    ...
    Widget::Children& Widget::getChildren() { return mNode->children_; }

    const Widget::Children& Widget::getChildren() const { return mNode->children_; }
    ...
    bool Widget::resizeEventTriggered() const { return triggerResizeEvent_; }

    UISystem* Widget::uiSystem() { return uiSystem_; }
    ...
    void Widget::update_event(const Inputs& inputs, float elapsed_time)
    {
        checkForResizeEvent();
        ...
        if(inputs.keyboard().is_key_pressed())
        {
            on_key_pressed(inputs);
        }

        if(mouse.is_button_up(Mouse::Button::Left))
        {
            dragging_ = false;
            on_mouse_drag_end_(mouse.x(), mouse.y());
            previous_x_position_ = mouse.x();
            previous_y_position_ = mouse.y();
        }

        if(dragging_)
        {
            if(previous_x_position_ != mouse.x() || previous_y_position_ != mouse.y())
            {
                previous_x_position_ = mouse.x();
                previous_y_position_ = mouse.y();
                on_mouse_drag_(mouse.x(), mouse.y());
            }
        }
        ...
        if(contains(mouse.x(), mouse.y()))
        {
            if(mouse.is_button_down(Mouse::Button::Left))
            {
                ...
                if(previous_x_position_ != mouse.x() || previous_y_position_ != mouse.y())
                {
                    previous_x_position_ = mouse.x();
                    previous_y_position_ = mouse.y();
                    on_mouse_drag_begin_(mouse.x(), mouse.y());
                }
            }
            if(!dragging_)
                on_mouse_over_(mouse.x(), mouse.y());
        }

        update(elapsed_time);

        for(auto& child : getChildren())
            child->mValue->update_event(inputs, elapsed_time);
    }

    bool Widget::isEnabled() const noexcept { return is_enabled_; }
