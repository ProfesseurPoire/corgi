...
{
public:
    AnimationSystem(ComponentPool<Animator>&       animators,
                    ComponentPool<SpriteRenderer>& sprite_renderers)
        : animators_(animators)
        , _sprite_renderers(sprite_renderers)
    {
    }
    ...
    void before_update(float elapsed_time) override
    {
        ...
        for(int i = 0; i < size; i++)
        {
            ...
            auto& entity = Game::instance().scene().entity_contiguous()[e.id_];
    ...
    void after_update(float) override
    {
        ...
        for(int i = 0; i < array_size; ++i)
        {
            ...
            sprite.flip_horizontal(animator.current_animation().flipped_x ^
                                   animator.is_flipped_);
