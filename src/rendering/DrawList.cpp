                        ...
                        Window*         window)
{
    ...
    _meshes.emplace_back(MeshToRender {mesh, material, model_matrix, window});
}
...
void DrawList::Text::setText(
    const std::vector<corgi::utils::text::ShapedGlyph>& shapedGlyph)
{
    shapedGlyphs_ = shapedGlyph;
    update();
}

void DrawList::Text::set_text(const std::string& text, bool force)
{
    if(force)
    {
        this->text = text;
...
Vector<int> DrawList::Text::charactersWidth(const std::string& text) const
{
    Vector<int> widths;

    for(auto shapedGlyph : shapedGlyphs_)
    {
        widths.push_back(static_cast<int>(shapedGlyph.advance.x));
    }

    return widths;
}

float DrawList::Text::textWidth(const std::string& text) const
{
    double width = 0.0;
...
void DrawList::Text::update()
{
    ...
    for(int i = 0; i < shapedGlyphs_.size(); i++)
    {
        ...
        text_real_width = math::ceil(static_cast<float>(real_advance_x));
    }
