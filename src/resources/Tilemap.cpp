...
namespace corgi
{
static void read_string(std::fstream& file, std::string& in)
{
    int size;
...
static void read_properties(std::fstream&                   file,
...
Tilemap::Property Tilemap::Object::property(const std::string& key) const
{
...
{
    file.read(reinterpret_cast<char*>(&tileset_info.firstgid),
              sizeof tileset_info.firstgid);
    file.read(reinterpret_cast<char*>(&tileset_info.columns),
              sizeof tileset_info.columns);
    file.read(reinterpret_cast<char*>(&tileset_info.image_height),
              sizeof tileset_info.image_height);
    file.read(reinterpret_cast<char*>(&tileset_info.image_width),
              sizeof tileset_info.image_width);
    file.read(reinterpret_cast<char*>(&tileset_info.margin), sizeof tileset_info.margin);
    file.read(reinterpret_cast<char*>(&tileset_info.tile_count),
              sizeof tileset_info.tile_count);
    file.read(reinterpret_cast<char*>(&tileset_info.tile_height),
              sizeof tileset_info.tile_height);
    file.read(reinterpret_cast<char*>(&tileset_info.tile_width),
              sizeof tileset_info.tile_width);
    file.read(reinterpret_cast<char*>(&tileset_info.spacing),
    ...
    for(size_t i = 0u; i < tiles_size; ++i)
    {
        ...
        file.read(reinterpret_cast<char*>(&tile_info.probablity),
...
Tilemap::Tilemap(const std::string& path, const std::string& identifier)
    : identifier(identifier)
{
    ...
    unsigned int tileset_info_size;
