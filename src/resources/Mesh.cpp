...
{
    return std::shared_ptr<Mesh>(new Mesh({{0, 0, 3}, {1, 3, 2}, {2, 5, 3}}));
}

std::shared_ptr<Mesh> Mesh::new_standard_mesh(std::vector<float>&&        vertices,
...
std::shared_ptr<Mesh> Mesh::new_standard_2D_mesh(std::vector<float>&&    vertices,
                                                 std::vector<unsigned>&& indices)
{
    return std::shared_ptr<Mesh>(
        new Mesh({{0, 0, 3}, {1, 3, 2}}, std::move(vertices), std::move(indices)));
}

std::shared_ptr<Mesh> Mesh::new_standard_2D_mesh()
{
    return std::shared_ptr<Mesh>(
        new Mesh({VertexAttribute {0, 0, 3}, VertexAttribute {1, 3, 2}}));
}

std::shared_ptr<Mesh> Mesh::new_standard_sphere(const float radius,
                                                const int   discretisation)
{
    // We need at least more than 1 slice
    assert(discretisation > 1);

    auto s = std::shared_ptr<Mesh>(
        new Mesh({VertexAttribute {0, 0, 3}, VertexAttribute {2, 3, 3}}));
    ...
    , indexes_(std::move(indices))
{
    ...
    RenderCommand::buffer_index_data(
        _ibo_index, indexes_.data(),
        static_cast<int>(indexes_.size() * sizeof(unsigned int)));

    for(auto& attribute : _attributes)
    {
        RenderCommand::enable_vertex_attribute(attribute.location);
        RenderCommand::vertex_attribute_pointer(attribute.location,
                                                vertex_size() * sizeof(float),
                                                attribute.offset, attribute.size);
    }
...
std::vector<Vertex> Mesh::add_vertices(int count)
{
    ...
    for(int i = 0; i < count; ++i)
        vertices.emplace_back(&vertices_[vertices_.size() - (count - i) * vertex_size()],
                              _attributes);
    return vertices;
}
...
{
    RenderCommand::buffer_vertex_subdata(
        _vbo_index, vertices_.data(), static_cast<int>(vertices_.size() * sizeof(float)));
}
...
void Mesh::addLine(unsigned index1, unsigned index2)
{
    indexes_.insert(indexes_.end(), {index1, index2});
}
...
bool Mesh::has_attribute(int location) const
{
    ...
    return std::find_if(_attributes.begin(), _attributes.end(),
                        [&](const VertexAttribute& v)
                        { return v.location == location; }) != _attributes.end();
}
...
void Mesh::set_indices(unsigned* i, int count)
{
    indexes_ = std::vector<unsigned>(i, i + count);
}
