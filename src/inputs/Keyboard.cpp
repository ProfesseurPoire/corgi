...
const std::vector<Key>& Keyboard::keyPressed() const
{
    return keyPressed_;
}

int Keyboard::getKeyModifiers() const
{
...
{
    return mKeys[static_cast<int>(k)];
}

bool Keyboard::key_down(Key k) const
{
    return mKeysDown[static_cast<int>(k)];
}

bool Keyboard::key_up(Key k) const
{
    return mKeysUp[static_cast<int>(k)];
}

bool Keyboard::is_key_pressed() const
{
    return _key_pressed_count > 0;
}
