...
void Settings::initialize(const SimpleString& path)
{
	setting_file_ = path;
...
const int& Settings::get_int(const SimpleString& name, const SimpleString& group)
{
	return document_.get_int(name.c_str(), group.c_str());
}

const float& Settings::get_float(const SimpleString& name, const SimpleString& group)
{
	return document_.get_float(name.c_str(), group.c_str());
}

const SimpleString& Settings::get_string(const SimpleString& name, const SimpleString& group)
{
