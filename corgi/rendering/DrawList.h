...
namespace corgi
{
    ...
    public:
        ...
        struct Text
        {
            ...
            corgi::Vector<corgi::utils::text::ShapedGlyph> shapedGlyphs_;
             ...
             * @return float 
             */
            float textWidth(const SimpleString& text) const;
             ...
             * @return Vector<float> 
             */
            Vector<int> charactersWidth(const SimpleString& text) const;

            void set_text(const SimpleString& text, bool force = false);

            void setText(const corgi::Vector<corgi::utils::text::ShapedGlyph>& glyphs);
        ...
        struct Rectangle
        {
            ...
            Rectangle(
                float p_x, float p_y, float p_width, float p_height, Material p_material)
                : x(p_x)
                , y(p_y)
                , width(p_width)
                , height(p_height)
        ...
        void
        add_mesh(const Mesh& mesh, const Matrix& model_matrix, const Material& material);
    ...
    private:
        ...
        Vector<Line>           lines_;
        Vector<NineSlice>      nine_slices_;
        Vector<Text>           texts_;
        Vector<DrawListSprite> sprites_;
        Vector<char>           resetStencilBuffer_;
        ...
        Vector<MeshToRender>                 _meshes;
        Vector<std::pair<DrawListType, int>> order_;
    };
}    // namespace corgi
