 ...
 */
    ...
    void Renderer::end_frame_buffer() { glBindFramebuffer(GL_FRAMEBUFFER, 0); }

    WindowDrawList& Renderer::window_draw_list() { return _window_draw_list; }
    WindowDrawList& Renderer::windowDrawList() { return _window_draw_list; }
    ...
    void Renderer::begin_default_frame_buffer() { glBindFramebuffer(GL_FRAMEBUFFER, 0); }
        ...
        if(scene.component_maps().contains<BoxCollider2D>())
        {
            for(auto& collider : *scene.component_maps().get<BoxCollider2D>())
            {
                auto  entity_id = EntityId(scene.component_maps()
                                               ...
                                               .at(i));
                auto& entity    = _current_scene->_entities_contiguous.at(entity_id.id_);
    ...
    static std::vector<const MeshRenderer*> sort_by_camera_layer(
        Scene& scene, const Camera& camera, const Vector<MeshRenderer>& meshes)
    {
    ...
    void Renderer::draw_scene()
    {
        ...
        if(Game::instance().window().isMinimized())
            return;
        ...
        for(auto& camera : *scene.component_maps().get<Camera>())
        {
            ...
            auto components = sort_by_camera_layer(scene, camera,
                                                   rendering_system._meshes.components());
            ...
            for(auto* component : components)
            {
                ...
                else
                {
                    ...
                    qsort(test_materials.data(), test_materials.size(),
                          sizeof(MaterialIndex), cmpfunc);
    ...
    void Renderer::initialize_opengl_state()
    {
        ...
        if(_opengl_state.enable_stencil_test)
        {
            ...
            set_stencil_enum(ssuccess_d_success,
                             _opengl_state.stencil_success_depth_success);
    ...
    void Renderer::begin_material(const Material& material)
    {
        ...
        if(material.enable_stencil_test())
        {
            ...
            set_stencil_enum(ssuccess_d_success,
                             material.stencil_success_depth_success());
    ...
    ProfilerInfo Renderer::profiler() const noexcept { return profiler_; }
	...
	profiler_.vertices_count+=mesh->vertex_count();*/
        ...
        switch(mesh->primitive_type())
        {
            case PrimitiveType::Triangles:
                ...
                glDrawElements(GL_TRIANGLES, mesh->indexes_.size(), GL_UNSIGNED_INT,
                               (void*)0);
                break;
            ...
            case PrimitiveType::Quads:
                ...
                glDrawElements(GL_QUADS, mesh->indexes_.size(), GL_UNSIGNED_INT,
                               (void*)0);
                break;

            case PrimitiveType::Lines:
                glDrawElements(GL_LINES, mesh->indexes().size(), GL_UNSIGNED_INT,
                               (void*)0);
                break;
        }
        //glBindVertexArray(0);
    ...
    void Renderer::draw_ns(const DrawList::NineSlice& n)
    {
        ...
        create_quad_lol(vertices, 0, n.x, n.y, n.left_border_, n.bottom_border_, 0.0f,
                        0.0f, n.left_border_uv_, a);
        ...
        create_quad_lol(vertices, 2, n.x + n.width - n.right_border_, n.y,
                        n.right_border_, n.bottom_border_, 1.0f - n.right_border_uv_,
                        0.0f, n.right_border_uv_, a);
        ...
        create_quad_lol(
            vertices, 5, n.x + n.width - n.right_border_, n.y + n.bottom_border_,
            n.right_border_, n.height - n.bottom_border_ - n.top_border_,
            1.0f - n.right_border_uv_, 0.0f + a, n.right_border_uv_, 1.0f - a - b);
    ...
    DrawList& Renderer::world_draw_list() { return _world_draw_list; }
