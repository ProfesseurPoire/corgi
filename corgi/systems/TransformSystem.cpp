...
namespace corgi
{
	...
	TransformSystem::TransformSystem(Scene& scene, ComponentPool<Transform>& transforms)
		: transforms_(transforms), _scene(scene)
	{}
	...
	void TransformSystem::update_component(Transform& transform, Entity& entity)
	{
		if (transform._dirty)	// Means the transform needs to be updated
		{
			...
			{
				if (entity.parent())
					transform._world_matrix = transforms_.get(entity.parent()->id())._world_matrix * local_matrix(transform);
				else
	...
	void TransformSystem::update(float elapsed_time)
	{
		...
		auto* entity_vector				= _scene._entities_contiguous.data();
		...
		for(int i=0; i< size; ++i)
		{
			...
			transforms[i]._depth = _scene._entities_contiguous[eid]._depth;
		}
		...
		std::sort(indexes.begin(), indexes.end(),
			[&](int& a, int& b)
			{
				return comps[a]._depth < comps[b]._depth;
			}
		);
