...
namespace corgi::math
{
                  ...
                  int                 position_offset)
    {
        ...
        for(int i = 0; i < indexes_size; i += 3)
        {
            Vec3 normal = get_triangle_normal(
                model_matrix *
                    Vec3(&vertices[indexes[i] * vertex_size + position_offset]),
                model_matrix *
                      ...
                      const Matrix& mat2)
    {
        ...
        for(int i = 0; i < edges_1_size; i++)
        {
            ...
            const float overlapvalue =
                GetOverlap(shape1projections.x, shape1projections.y, shape2projections.x,
                           shape2projections.y);
        ...
        Vec3&  intersectionNormal)
    {
        ...
        for(int i = 0; i < indexes_size; i += 3)
        {
            ...
            Vec3 posB =
                model_matrix * Vec3(&vertices[indexes[i + 1] * vertex_size + offset]);
            Vec3 posC =
                model_matrix * Vec3(&vertices[indexes[i + 2] * vertex_size + offset]);
            ...
            if(denom != 0)
            {
                ...
                Vec3 P(ray.start.x + t * ray.direction.x,
    ...
    Vec3 project_points(const Vec3* points, const int points_size, const Vec3& axis)
    {
                                 ...
                                 Vec3&               intersection_normal)
    {
        ...
        for(int i = 0; i < indexes_size; i += 3)
        {
            ...
            denom = (-start.x * normal.x - start.y * normal.y - start.z * normal.z - D) /
                    denom;
