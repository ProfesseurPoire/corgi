...
namespace corgi
{
    ...
    public:
        ...
        static SharedPtr<Mesh> new_standard_mesh();

        static SharedPtr<Mesh> new_standard_mesh(std::vector<float>&& vertices, std::vector<unsigned int>&& indices);

        /*!
		 * @brief A standard 2D mesh wil have 3 float for the position (I know, I probably should use 2) and 2 float for uv coordinates
		 */
        static SharedPtr<Mesh> new_standard_2D_mesh(std::vector<float>&& vertices, std::vector<unsigned int>&& indices);

        static SharedPtr<Mesh> new_standard_2D_mesh();

        // Creates a standard sphere, with a position and normal attributes
        static SharedPtr<Mesh> new_standard_sphere(float radius, int discretisation);
