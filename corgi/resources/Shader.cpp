...
namespace corgi
{
	Shader::Shader(const SimpleString& path, const SimpleString& identifier)
	{
		//log_info("Shader Constructor for "+path);
		std::string pathh = path.c_str();

		if(pathh.find("_vs") != SimpleString::npos)
			id_ = glCreateShader(GL_VERTEX_SHADER);
		else if(pathh.find("_fs") != SimpleString::npos)
		...
		if(success==GL_FALSE)
		{
			...
			SimpleString str=" Error while loading " + path + "shader";
