...
float Axis::value() const
{
    ...
    if (inputs_.keyboard().key(positive_)  || ( use_hat_ &&is_positive_hat_triggered()))
    {
        return 1.0f;
    }

    if (inputs_.keyboard().key(negative_) ||(use_hat_ && is_negative_hat_triggered()))
    {
        return -1.0f;
    }
...
Axis::Axis(const Inputs& inputs , Key positive, Key negative) : 
    inputs_(inputs), positive_(positive), negative_(negative){}
